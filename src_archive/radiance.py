"""
2-Phase Daylight Coefficient (DDS) Method for Annual Irradiance Simulation using Pyradiance

This module implements the 2-phase DDS workflow for calculating annual irradiance
using the pyradiance Python wrapper for Radiance. The workflow follows the standard
Radiance 2-phase method.

NOTE: The pyradiance API function calls (e.g., pr.oconv(), pr.rfluxmtx(), etc.) may need
to be adjusted based on the actual pyradiance package API. Please refer to the pyradiance
documentation at https://lbnl-eta.github.io/pyradiance/ for the correct function signatures.

Installation:
    conda activate radiance_testing
    pip install pyradiance numpy pandas pyarrow
...
try:
    ...
    raise ImportError(
        "pyradiance is not installed. Install it with: pip install pyradiance"
    ) from exc
...
def build_octree(
    ...
    octree_bytes = pr.oconv(*input_files)
...
def run_rfluxmtx(
    ...
    if use_accelerad:
        ...
        # Verify it's in PATH
        if find_accelerad_command(cmd_name) is None:
            raise FileNotFoundError(
                f"Accelerad command '{cmd_name}' not found in PATH. "
                f"Please ensure Accelerad is installed and in PATH."
            )
        cmd_path = cmd_name  # Use bare name, not full path
    else:
        cmd_name = "rfluxmtx"
        cmd_path = os.path.join(pr.BINPATH, cmd_name) if os.path.exists(os.path.join(pr.BINPATH, cmd_name)) else cmd_name
...
def run_gendaymtx(
    ...
    gendaymtx_path = os.path.join(pr.BINPATH, "gendaymtx")
...
def run_dctimestep_rmtxop(
    ...
    dctimestep_output_bytes = pr.dctimestep(input_matrix, input_sky)
    
    # Run rmtxop with conversion coefficients (47.4, 119.9, 11.6) for RGB to lux
    result_bytes = pr.rmtxop(
...
def create_sun_discs(radiance_project_dir: str, radiance_surface_key: str) -> None:
    ...
    cnt_path = os.path.join(pr.BINPATH, "cnt")
    rcalc_path = os.path.join(pr.BINPATH, "rcalc")
...
def epw2wea(
    ...
    epw2wea_path = os.path.join(pr.BINPATH, "epw2wea")
